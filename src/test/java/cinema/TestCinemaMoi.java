...
class TestCinemaMoi {
	...
	void initData()
	{
		...
		Collections.addAll(movies,
				new MovieMoi ("Joker", 2019, 165, tod),								//0
				new MovieMoi ("Parasite", 2019, 165),								//1
				new MovieMoi ("Interstellar", 2014, 179),							//2
				new MovieMoi ("Grand Torino", 2019, 116, clint),					//3
				new MovieMoi ("Impitoyable", 1992, 131),							//4
				new MovieMoi ("American Sniper", 2014, 133, clint),					//5
				new MovieMoi ("Very Bad Trip", 2009, 100),						//6
				new MovieMoi ("Avengers Infinity War", 2018, 160),
				new MovieMoi ("Avengers Engame",2019, 182),
				new MovieMoi ("Captain Marvel", 2019, 125),
				new MovieMoi ("Avengers", 2012, 143),
				new MovieMoi ("Avengers L'�re D'Ultron",2015, 142),
				new MovieMoi ("Night of the Day of the Dawn of the Son of the Bride of the Return of the Revenge of the Terror of the Attack of the Evil, Mutant, Alien, Flesh Eating, Hellbound, Zombified Living Dead Part", 1991 )
				);
...
//				);
	...
	{
		SortedSet<MovieMoi> sortedMovies = new TreeSet<>((m1,m2) -> -1);
	...
	void testSortMovies()
	{
		Collections.sort(movies, 
						 Comparator.comparing(MovieMoi::getAnnee, Comparator.reverseOrder())
						       .thenComparing(MovieMoi::getTitre));
//						 	   .thenComparing(m->m.getTitre()));
	...
	void testFirstLastYearAvengersMovie()
	{
		var stat = movies.stream()
				  .filter(m -> m.getTitre().contains("Avengers"))
				  //.forEach(System.out::println)
				  .mapToInt(MovieMoi::getAnnee) 		// (MovieMoi::getAnnee)  <=>  (m -> m.getAnnee())
	...
	void testChronoAvengersMovie()
	{
		...
		.collect(Collectors.toCollection(() -> new TreeSet<>(
				Comparator.comparing(MovieMoi::getAnnee)
						)));
	...
	{
		movies.stream()
		.filter((MovieMoi m) -> m.getAnnee() > 1900)
	...
	void nbFilmParAnnee()
	{
		var res =movies.stream()
			.collect(Collectors.groupingBy(MovieMoi::getAnnee, Collectors.counting()));   // nombre de ... => counting 
	...
	void nbFilmParRealisateurV1()    
	{
		var res =movies.stream()
				...
				.collect(Collectors.groupingBy(MovieMoi::getDirector, Collectors.counting()));   
			System.out.println(res);
	}
	...
	void nbFilmParRealisateurV3()
	{
		var filmographies = movies.stream()
				.filter(m -> Objects.nonNull(m.getDirector()))
				.collect(Collectors.groupingBy(MovieMoi::getDirector,
						Collectors.toCollection(
								() -> new TreeSet<>(Comparator.comparing(MovieMoi::getAnnee,
